...
class TestSpacedRepetitionDueCardsEndpoint:
            ...
            "user_flashcard_spaced_repetition": [
                {
                    ...
                    "current_interval": 3,
                    "last_reviewed_at": (datetime.utcnow() - timedelta(days=3)).isoformat() + "Z"
                }
            ]
        }
            ...
            "user_flashcard_spaced_repetition": [
                {
                    "due_date": (datetime.utcnow() + timedelta(days=1)).isoformat() + "Z",
                    "current_interval": 2,
                    "last_reviewed_at": (datetime.utcnow() - timedelta(days=1)).isoformat() + "Z"
                }
            ]
        }
    ...
    def test_get_due_flashcards_success(self, mock_get_supabase):
        ...
        # Act
        response = self.client.get("/api/v1/spaced-repetition/due-cards", headers=self.auth_headers)
    ...
    def test_get_due_flashcards_invalid_limit(self):
        ...
        response = self.client.get(
            "/api/v1/spaced-repetition/due-cards?limit=0",
    ...
    def test_get_due_flashcards_with_limit(self, mock_get_supabase):
        ...
        response = self.client.get(
            "/api/v1/spaced-repetition/due-cards?limit=1", 
            headers=self.auth_headers
        )
    ...
    def test_get_due_flashcards_unauthorized_invalid_token(self, mock_get_supabase):
        ...
        # Act
        response = self.client.get("/api/v1/spaced-repetition/due-cards", headers=invalid_headers)
