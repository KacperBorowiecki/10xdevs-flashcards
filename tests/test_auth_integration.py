...
@pytest.mark.skip(
    "TODO: Fix integration test authorization, mocking and TestClient API issues"
)
class TestAuthViews:
    ...
    def test_login_redirect_when_authenticated(self):
        ...
        with patch(
            "src.services.auth_service.AuthService.is_authenticated", return_value=True
        ):
    ...
    def test_login_invalid_credentials(self):
        ...
        form_data = {"email": "test@error.com", "password": "wrongpassword"}
    ...
    def test_login_unconfirmed_email(self):
        ...
        form_data = {"email": "test@unconfirmed.com", "password": "validpassword123"}
        response = client.post("/login", data=form_data)
    ...
    def test_login_network_error(self):
        ...
        form_data = {"email": "test@network.com", "password": "validpassword123"}
        response = client.post("/login", data=form_data)
    ...
    def test_redirect_after_login_with_next_param(self):
        ...
        response = client.post(
            "/login?next=/dashboard", data=form_data, allow_redirects=False
        )
        assert response.status_code == 302
        assert response.headers["location"] == "/dashboard"
    ...
    def test_auth_exceptions(self):
        ...
        assert (
            EmailNotConfirmedError().message
