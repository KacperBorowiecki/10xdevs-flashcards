...
from src.api.v1.routers.spaced_repetition_router import (
    router as spaced_repetition_router,
)
...
async def global_exception_handler(request: Request, exc: Exception):
    """Global exception handler for unexpected errors."""
    logger.error(
        f"Unexpected error on {request.method} {request.url}: {str(exc)}", exc_info=True
    )
...
app.include_router(spaced_repetition_router, prefix="/api/v1")
