...
class AiGenerationService:
    ...
    async def get_user_generation_stats(
        ...
        try:
            ...
            count_response = self.supabase.table("ai_generation_events") \
                .select("*", count="exact") \
            ...
            data_response = self.supabase.table("ai_generation_events") \
                .select("*") \
                .eq("user_id", str(user_id)) \
            ...
            logger.info(f"Retrieved {len(items)} AI generation events for user {user_id} (page {page}/{pages})")
            ...
            logger.error(f"Error retrieving AI generation stats for user {user_id}: {str(e)}")
