...
class SpacedRepetitionService:
    ...
    async def _validate_flashcard_access(
        ...
        try:
            ...
            response = (
                ...
                .eq("status", "active")  # Filter active status in query for better performance
                .single()  # Use single() for single record queries
                .execute()
            )
            
            if not response.data:
                raise ValueError("Flashcard not found, doesn't belong to user, or is not active")
            
            flashcard = response.data
            
            # Additional security: Check if flashcard is too old (potential abuse)
            created_at = datetime.fromisoformat(flashcard["created_at"].replace('Z', '+00:00'))
            if created_at.tzinfo:
                created_at = created_at.replace(tzinfo=None)
            
            days_since_creation = (datetime.utcnow() - created_at).days
            if days_since_creation > 3650:  # 10 years - reasonable limit
                logger.warning(f"Attempt to review very old flashcard | flashcard_id={flashcard_id} | days_old={days_since_creation}")
    ...
    async def _get_or_create_repetition_record(
        ...
        try:
            ...
            response = (
                self.supabase
                .table("user_flashcard_spaced_repetition")
            ...
            logger.debug(f"No existing repetition record found for flashcard {flashcard_id}")
            return None

    def _validate_review_frequency(self, existing_record: Optional[Dict[str, Any]]) -> None:
        """
        ...
        if last_review_count > 50:
            ...
            if hours_since_creation < 24:  # Within 24 hours
                logger.warning(f"Rate limit exceeded for flashcard review | reviews={last_review_count}")
                raise ValueError("Too many reviews for this flashcard today. Please try again later.")
    ...
    async def review_flashcard(
        self, 
        command: ReviewFlashcardCommand
    ) -> SpacedRepetitionReviewResponse:
        """
        ...
        try:
            ...
            sanitized_rating = self._sanitize_performance_rating(command.performance_rating)
            ...
            current_interval = existing_record.get("current_interval", 1) if existing_record else 1
            ...
            now = datetime.utcnow()
            review_count = (existing_record.get("data_extra", {}).get("review_count", 0) + 1) if existing_record else 1
            ...
            response = (
                self.supabase
                .table("user_flashcard_spaced_repetition")
                .upsert(upsert_data, on_conflict="user_id,flashcard_id")  # Specify conflict columns
                .execute()
            )
            ...
            result = SpacedRepetitionReviewResponse(
                ...
                due_date=datetime.fromisoformat(updated_record["due_date"].replace('Z', '+00:00')).replace(tzinfo=None),
                current_interval=updated_record["current_interval"],
                last_reviewed_at=datetime.fromisoformat(updated_record["last_reviewed_at"].replace('Z', '+00:00')).replace(tzinfo=None),
                data_extra=updated_record.get("data_extra"),
                created_at=datetime.fromisoformat(updated_record["created_at"].replace('Z', '+00:00')).replace(tzinfo=None),
                updated_at=datetime.fromisoformat(updated_record["updated_at"].replace('Z', '+00:00')).replace(tzinfo=None)
            )
    ...
    async def get_due_flashcards(
        ...
        try:
            ...
            for flashcard_data in response.data:
                ...
                sr_record = sr_data[0] if isinstance(sr_data, list) and sr_data else sr_data
                if not sr_record:
                    continue
                ...
                try:
                    ...
                    logger.warning(f"Invalid due_date format for flashcard {flashcard_data.get('id')}")
                    continue
                ...
                if due_date <= current_time:
                    ...
                    if sr_record.get("last_reviewed_at"):
                        try:
                            ...
                            logger.warning(f"Invalid last_reviewed_at format for flashcard {flashcard_data.get('id')}")
                    ...
                    clean_flashcard_data = {k: v for k, v in flashcard_data.items() 
            ...
            logger.warning(f"Validation error getting due flashcards for user {user_id}: {str(e)}")
            raise
        except Exception as e:
