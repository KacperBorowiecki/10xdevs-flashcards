...
from src.dtos import AIGenerateFlashcardsRequest, AIGenerateFlashcardsResponse, FlashcardResponse
...
class AIService:
    ...
    async def generate_flashcards_from_text(
        ...
        try:
            ...
            logger.info(f"Successfully completed AI flashcard generation for user {user_id}")
            return response
    ...
    async def _create_source_text(self, text_content: str, user_id: uuid.UUID) -> dict:
        ...
        try:
            ...
            logger.error(f"Database error creating source text for user {user_id}: {str(e)}")
            raise AIServiceError(
                operation="create_source_text",
    ...
    async def _create_flashcard_records(
        ...
        try:
            ...
            logger.error(f"Database error creating flashcards for user {user_id}: {str(e)}")
            raise AIServiceError(
                operation="create_flashcards", 
                details=f"Database operation failed: {str(e)}",
                user_id=user_id
    ...
    async def _create_ai_generation_event(
        ...
        try:
            ...
            logger.error(f"Database error creating AI event for user {user_id}: {str(e)}")
            raise AIServiceError(
                operation="create_ai_event",
                details=f"Database operation failed: {str(e)}",
                user_id=user_id
