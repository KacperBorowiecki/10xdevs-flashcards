...
from src.dtos import LLMGenerateResponse, LLMFlashcardSuggestion
        ...
        try:
            ...
            if not response.choices:
                raise LLMServiceError(
                    ...
                    details="No choices in LLM response"
                )
            ...
            if not content:
                raise LLMServiceError(
                    operation="parse_response",
                    details="Empty content in LLM response"
                )
            ...
            for card_data in parsed_content["flashcards"]:
                ...
                try:
                    ...
                    logger.warning(f"Failed to create flashcard from data {card_data}: {e}")
                    continue
            ...
            logger.info(f"Successfully generated {len(flashcards)} flashcards using {self.settings.LLM_MODEL}")
