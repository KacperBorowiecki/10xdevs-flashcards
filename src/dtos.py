...
class LLMFlashcardSuggestion(BaseModel):
    ...
    front_content: str = Field(
        ..., description="Front content of the suggested flashcard"
    )
    back_content: str = Field(
        ..., description="Back content of the suggested flashcard"
    )
...
class LLMGenerateResponse(BaseModel):
    ...
    flashcards: List[LLMFlashcardSuggestion] = Field(
...
class FlashcardManualCreateRequest(BaseModel):
    front_content: str = Field(
        ..., max_length=500, description="Front content of the flashcard."
    )
    back_content: str = Field(
        ..., max_length=1000, description="Back content of the flashcard."
    )
...
class FlashcardPatchRequest(BaseModel):
    front_content: Optional[str] = Field(
        None, max_length=500, description="Optional new front content."
    )
    back_content: Optional[str] = Field(
        None, max_length=1000, description="Optional new back content."
    )
    status: Optional[FlashcardStatusEnum] = Field(
        None,
        description="Optional new status, e.g., for AI suggestions 'active' or 'rejected'.",
    )
...
class AIGenerateFlashcardsRequest(BaseModel):
    text_content: str = Field(
        ...
        min_length=1000,
        max_length=10000,
        description="Text content to generate flashcards from (1000-10000 characters).",
    )
...
class DueFlashcardResponse(FlashcardResponse):
    ...
    repetition_data: Optional[RepetitionData] = Field(
        None, description="Spaced repetition data if available."
    )
...
class SubmitReviewRequest(BaseModel):
    ...
    performance_rating: int = Field(
        ...
        ge=1,
        le=5,
        description="User's performance rating for the flashcard (e.g., 1-5).",
    )  # Zakładamy skalę 1-5
...
class AIGenerationSummary(BaseModel):
    ...
    total_generated: int = Field(
        ..., ge=0, description="Łączna liczba wygenerowanych fiszek"
    )
    total_accepted: int = Field(
        ..., ge=0, description="Łączna liczba zaakceptowanych fiszek"
    )
...
class DashboardStats(BaseModel):
    ...
    total_flashcards: int = Field(
        ..., ge=0, description="Łączna liczba aktywnych fiszek"
    )
    due_cards_today: int = Field(
        ..., ge=0, description="Liczba fiszek do powtórki dziś"
    )
...
class DashboardContext(BaseModel):
    ...
    error_message: Optional[str] = Field(
        None, description="Komunikat błędu jeśli wystąpił"
    )
...
if __name__ == "__main__":
    ...
    manual_card_data = {
        ...
        "back_content": "A modern Python web framework.",
    }
