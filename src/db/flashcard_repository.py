...
class FlashcardRepository:
    ...
    def get_ai_generation_event_by_source_text(
        ...
        try:
            ...
            logger.error(f"Error getting AI generation event for source_text {source_text_id}: {str(e)}")
            raise
    ...
    async def get_flashcard_with_concurrent_check(
        ...
        try:
            ...
            logger.error(f"Error getting flashcard with concurrent check {flashcard_id}: {str(e)}")
            raise
    ...
    async def execute_transaction_simulation(
        ...
        try:
            ...
            logger.error(f"Transaction simulation failed at operation {len(executed_operations)}: {str(e)}")
            await self._compensate_operations(executed_operations)
            raise
    
    async def _compensate_operations(self, executed_operations: List[Dict[str, Any]]) -> None:
        """
            ...
            for operation in reversed(executed_operations):
                ...
                if op_type == 'insert':
                    ...
                    if result and 'id' in result:
                        try:
                            self.supabase.table(operation['table']).delete().eq('id', result['id']).execute()
                            logger.info(f"Compensated insert operation for {operation['table']} id {result['id']}")
                        except Exception as comp_error:
                            logger.error(f"Failed to compensate insert: {str(comp_error)}")
