...
from src.core.exceptions import ValidationError as AuthValidationError
from src.core.exceptions import (
...
async def login_page(request: Request, error: Optional[str] = None):
    ...
    return templates.TemplateResponse(
        "auth.html", {"request": request, "mode": "login", "error_message": error}
    )
...
async def register_page(request: Request, error: Optional[str] = None):
    ...
    return templates.TemplateResponse(
        "auth.html", {"request": request, "mode": "register", "error_message": error}
    )
...
async def login_submit(
    ...
    try:
        ...
        try:
            ...
            if (
                hasattr(auth_response.user, "email_confirmed_at")
                and auth_response.user.email_confirmed_at is None
            ):
...
async def register_submit(
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"User {register_data.email} registered and logged in automatically"
            )
            return response
...
async def verify_email(request: Request, token: str, type: Optional[str] = "signup"):
    """
...
async def resend_verification_email(request: Request, email: str = Form(...)):
    ...
    try:
        ...
        success_message = (
            "Email weryfikacyjny został wysłany ponownie. Sprawdź swoją skrzynkę."
        )
        return templates.TemplateResponse(
            "auth.html",
            {"request": request, "mode": "login", "success_message": success_message},
        )

    except Exception as e:
        logger.error(f"Error resending verification email: {str(e)}")
        error_message = (
            "Nie udało się wysłać emaila weryfikacyjnego. Spróbuj ponownie później."
        )
        return templates.TemplateResponse(
            "auth.html",
            {"request": request, "mode": "login", "error_message": error_message},
...
async def forgot_password(request: Request, email: str = Form(...)):
    ...
    try:
        ...
        response = supabase.auth.reset_password_email(
            email=email, redirect_to=f"{request.base_url}verify-email"
        )
...
async def reset_password_submit(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "reset_password.html",
            {"request": request, "token": token, "error_message": error_message},
        )
