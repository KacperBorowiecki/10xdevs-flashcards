...
async def flashcards_view(
    request: Request,
    source: Optional[str] = Query(None, description="Filter by flashcard source (manual, ai_suggestion)"),
    ...
    try:
        ...
        logger.info(f"Flashcards view accessed by: {user_email}, "
        ...
        logger.error(f"Unexpected error in flashcards view for user {user_email}: {str(e)}")
