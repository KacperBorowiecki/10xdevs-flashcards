...
async def dashboard_page(
    ...
    try:
        ...
        dashboard_context = await dashboard_service.get_dashboard_context(user_id, user_email)
        
        logger.info(f"Dashboard accessed by: {user_email} with stats: "
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                ...
                "timestamp": dashboard_stats.model_dump() if hasattr(dashboard_stats, 'model_dump') else None
            }
        )
        
    except ValueError as e:
        logger.error(f"Invalid user ID format in refresh stats: {user_id_str}, error: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid user ID format"
        )
    except DashboardServiceError as e:
        logger.error(f"Dashboard service error in refresh stats for user {user_email}: {e.details}")
        raise HTTPException(
        ...
        logger.error(f"Unexpected error in refresh stats for user {user_email}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
