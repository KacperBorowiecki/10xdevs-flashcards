...
async def get_due_flashcards(
    ...
    spaced_repetition_service: Annotated[
        SpacedRepetitionService, Depends(get_spaced_repetition_service)
    ],
    limit: int = Query(
        default=20,
        ge=1,
    ...
    try:
        ...
        if not current_user_id:
            logger.warning(
                f"Unauthenticated request for due cards | operation={operation}"
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
...
async def submit_flashcard_review(
    ...
    try:
        ...
        if not current_user_id:
            logger.warning(
                f"Unauthenticated request for review submission | operation={operation}"
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="User authentication required",
        ...
        if not isinstance(review_request.performance_rating, int) or not (
            1 <= review_request.performance_rating <= 5
        ):
            logger.warning(
        ...
        if "not found" in error_message.lower():
            ...
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Flashcard is not active"
            )
