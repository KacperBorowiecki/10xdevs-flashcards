...
class RepetitionData(BaseModel):
    ...
    current_interval: int = Field(description="Current spaced repetition interval in days")
    last_reviewed_at: Optional[datetime] = Field(
        default=None, 
        description="Timestamp of last review (null if never reviewed)"
    )
...
class SpacedRepetitionReviewResponse(BaseModel):
    ...
    data_extra: Optional[Dict[str, Any]] = Field(
        default=None, 
        description="Additional spaced repetition data"
    )
