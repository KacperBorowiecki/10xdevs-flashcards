...
class FlashcardManualCreateRequest(BaseModel):
    ...
    front_content: str = Field(
        ..., description="Front content of the flashcard.", min_length=1, max_length=500
    )
    back_content: str = Field(
        ..., description="Back content of the flashcard.", min_length=1, max_length=1000
    )
...
class ListFlashcardsQueryParams(BaseModel):
    ...
    status: Optional[FlashcardStatusEnum] = Field(
        default=FlashcardStatusEnum.ACTIVE, description="Filter by flashcard status"
    )
    source: Optional[FlashcardSourceEnum] = Field(
        default=None, description="Filter by flashcard source"
    )
